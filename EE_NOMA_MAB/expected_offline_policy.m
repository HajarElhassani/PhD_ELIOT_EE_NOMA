...
function mu_best = expected_offline_policy(arms,Pmax,Pc,threshold1,threshold2,sigma1,sigma2,var_h1,var_h2)

% Expected reward calculation for each arm
for i=1:length(arms)
mu(i)=expectedValue_reward(arms(i,:),threshold1,threshold2,Pmax,sigma1,sigma2,var_h1,var_h2);
end
mu_a=(log2(1+threshold1)+log2(1+threshold2))*mu./(Pmax*arms(:,2)'+Pc);
